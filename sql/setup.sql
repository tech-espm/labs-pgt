...
CREATE TABLE IF NOT EXISTS `labs_pgt`.`pgt` (
  `pgt_id` INT NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `labs_pgt`.`conta_pgt` (
  ...
  CONSTRAINT `conta_pgt_conta` FOREIGN KEY (`conta_id`) REFERENCES `labs_pgt`.`conta` (`conta_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `conta_pgt_pgt` FOREIGN KEY (`pgt_id`) REFERENCES `labs_pgt`.`pgt` (`pgt_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `labs_pgt`.`etapa` (
  ...
  CONSTRAINT `etapa_pgt` FOREIGN KEY (`etapa_pgt_id`) REFERENCES `labs_pgt`.`pgt` (`pgt_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `labs_pgt`.`cronograma` (
  ...
  CONSTRAINT `cronograma_pgt` FOREIGN KEY (`cronograma_pgt_id`) REFERENCES `labs_pgt`.`pgt` (`pgt_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `labs_pgt`.`formularios` (
  ...
  CONSTRAINT `formulario_pgt` FOREIGN KEY (`formulario_pgt_id`) REFERENCES `labs_pgt`.`pgt` (`pgt_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `formulario_conta_autor` FOREIGN KEY (`formulario_conta_autor`) REFERENCES `labs_pgt`.`conta` (`conta_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE = InnoDB;
...
values
  ('superv@espm.com.br', 'admin', 'senha123', 1);